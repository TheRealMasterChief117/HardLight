...
public sealed class ShipyardGridSaveSystem : EntitySystem
{
    ...
    private async Task<EntityUid?> MoveAndCleanGrid(EntityUid originalGridUid, MapId targetMapId)
    {
        try
        {
            // Move the grid to the temporary map
            ...
            _sawmill.Info($"Moved grid {originalGridUid} to temporary map {targetMapId}");
    ...
    /// This includes session-specific data, vending machine contents, runtime state, etc.
    /// </summary>
    private void CleanGridForSaving(EntityUid gridUid)
    {
        ...
        _sawmill.Info($"Grid cleanup complete: removed {componentsRemoved} components from {allEntities.Count} entities");
    }
